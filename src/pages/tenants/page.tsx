...
const timezoneOptions = ['America/Costa_Rica', 'America/Panama', 'America/Guatemala', 'America/Mexico_City', 'America/Bogota', 'America/Lima', 'America/Santiago', 'America/Argentina/Buenos_Aires'];
...
export default function TenantsPage() {
  ...
  const handleSave = async () => {
    ...
    if (!formStatus) { setFormError('El estado es requerido'); return; }

    setSaving(true);
    setFormError('');

    const settings: TenantSettings = {
      logo_url: formSettings.logo_url?.trim() || undefined,
      primary_email: formSettings.primary_email?.trim() || undefined,
      phone: formSettings.phone?.trim() || undefined,
      website: formSettings.website?.trim() || undefined,
      timezone: formSettings.timezone || 'America/Costa_Rica',
      language: formSettings.language || 'es',
      currency: formSettings.currency || 'USD',
      plan: formSettings.plan || 'Free',
    };

    const hasSettings = Object.values(settings).some((v) => v !== undefined && v !== '' && v !== 'Free' && v !== 'es' && v !== 'USD' && v !== 'America/Costa_Rica');
  ...
  const formatDate = (d: string) => new Date(d).toLocaleDateString('es-CR', { year: 'numeric', month: 'short', day: 'numeric' });
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-lg font-semibold text-foreground-200 mb-2">Error al cargar tenants</h2>
    ...
    <AppLayout>
      <div className="animate-fade-in space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: 'Total tenants', value: tenants.length, icon: 'ri-building-4-line', bg: 'bg-primary-500/10', text: 'text-primary-400' },
            { label: 'Activos', value: activeTenants.length, icon: 'ri-checkbox-circle-line', bg: 'bg-emerald-500/10', text: 'text-emerald-400' },
            { label: 'Suspendidos', value: suspendedTenants.length, icon: 'ri-pause-circle-line', bg: 'bg-amber-500/10', text: 'text-amber-400' },
            { label: 'Total usuarios', value: tenants.reduce((s, t) => s + t.user_count, 0), icon: 'ri-team-line', bg: 'bg-accent-500/10', text: 'text-accent-400' },
          ].map((stat) => (
        ...
        {tenants.length === 0 && !loading ? (
          <div className="glass-panel rounded-2xl p-16 text-center">
            ...
            <h2 className="text-lg font-semibold text-foreground-200 mb-2">No hay tenants creados</h2>
          ...
          <div className="glass-panel rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-secondary-500/10 flex flex-col sm:flex-row gap-3">
              ...
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as FilterStatus)} className="h-9 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-300 outline-none focus:border-primary-500/40">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-secondary-500/10">
                    ...
                    <th className="text-center px-4 py-3 text-xs font-medium text-foreground-500 uppercase tracking-wider">Clientes</th>
                    <th className="text-center px-4 py-3 text-xs font-medium text-foreground-500 uppercase tracking-wider">Usuarios</th>
                ...
                <tbody>
                  {filtered.map((t) => {
                    ...
                    return (
                      <tr key={t.id} className="border-b border-secondary-500/5 hover:bg-background-100/50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-center"><span className="text-sm font-medium text-foreground-300">{t.country_count}</span></td>
                        <td className="px-4 py-3.5 text-center"><span className="text-sm font-medium text-foreground-300">{t.warehouse_count}</span></td>
                        <td className="px-4 py-3.5 text-center"><span className="text-sm font-medium text-foreground-300">{t.client_count}</span></td>
                        <td className="px-4 py-3.5 text-center"><span className="text-sm font-medium text-foreground-300">{t.user_count}</span></td>
              ...
              {filtered.length === 0 && tenants.length > 0 && (
                <div className="py-16 text-center">
                  ...
                  <p className="text-sm text-foreground-500">No se encontraron tenants</p>
                </div>
              )}
            </div>
            <div className="p-4 border-t border-secondary-500/10 flex items-center justify-between">
              <span className="text-xs text-foreground-600">{filtered.length} de {tenants.length} tenants</span>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative glass-panel-strong rounded-2xl w-full max-w-xl p-6 animate-scale-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-foreground-200">{editing ? 'Editar tenant' : 'Crear tenant'}</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-foreground-400 mb-1.5">Nombre de empresa <span className="text-red-400">*</span></label>
                  <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-200 outline-none focus:border-primary-500/40 transition-all" placeholder="Ej: OLO Logistics" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-foreground-400 mb-1.5">Codigo <span className="text-red-400">*</span></label>
                  <input
                    type="text"
                    value={formCode}
                    onChange={(e) => setFormCode(e.target.value.toUpperCase())}
                    disabled={!!editing}
                    className={`w-full h-10 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-200 outline-none focus:border-primary-500/40 transition-all font-mono ${editing ? 'bg-secondary-500/5 text-foreground-500 cursor-not-allowed' : 'bg-background-100'}`}
                    placeholder="OLO"
                    maxLength={10}
                  />
                  {editing && <p className="text-2xs text-foreground-600 mt-1">El codigo no se puede modificar</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={formStatus} onChange={(e) => setFormStatus(e.target.value)} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-300 outline-none focus:border-primary-500/40">
                    <option value="active">Activo</option>
                    <option value="suspended">Suspendido</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-medium text-foreground-400 mb-1.5">Plan</label>
                  <select value={formSettings.plan} onChange={(e) => setFormSettings({ ...formSettings, plan: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-300 outline-none focus:border-primary-500/40">
              ...
              <div className="border-t border-secondary-500/10 pt-4 mt-2">
                <p className="text-xs font-medium text-foreground-400 mb-3">Informacion adicional (opcional)</p>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-foreground-400 mb-1.5">Email principal</label>
                    <input type="email" value={formSettings.primary_email || ''} onChange={(e) => setFormSettings({ ...formSettings, primary_email: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-200 outline-none focus:border-primary-500/40 transition-all" placeholder="contacto@empresa.com" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-foreground-400 mb-1.5">Telefono</label>
                    <input type="text" value={formSettings.phone || ''} onChange={(e) => setFormSettings({ ...formSettings, phone: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-200 outline-none focus:border-primary-500/40 transition-all" placeholder="+506 2222-3333" />
                  </div>
                </div>

                <div className="mt-4">
                  <label className="block text-xs font-medium text-foreground-400 mb-1.5">Sitio web</label>
                  <input type="text" value={formSettings.website || ''} onChange={(e) => setFormSettings({ ...formSettings, website: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-200 outline-none focus:border-primary-500/40 transition-all" placeholder="https://empresa.com" />
                </div>

                <div className="grid grid-cols-3 gap-4 mt-4">
                  <div>
                    <label className="block text-xs font-medium text-foreground-400 mb-1.5">Idioma</label>
                    <select value={formSettings.language} onChange={(e) => setFormSettings({ ...formSettings, language: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-300 outline-none focus:border-primary-500/40">
                  ...
                  <div>
                    ...
                    <select value={formSettings.timezone} onChange={(e) => setFormSettings({ ...formSettings, timezone: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-300 outline-none focus:border-primary-500/40">
                      {timezoneOptions.map((t) => <option key={t} value={t}>{t.replace('America/', '')}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-foreground-400 mb-1.5">Moneda</label>
                    <select value={formSettings.currency} onChange={(e) => setFormSettings({ ...formSettings, currency: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-300 outline-none focus:border-primary-500/40">
                ...
                <div className="mt-4">
                  <label className="block text-xs font-medium text-foreground-400 mb-1.5">URL del logo</label>
                  <input type="text" value={formSettings.logo_url || ''} onChange={(e) => setFormSettings({ ...formSettings, logo_url: e.target.value })} className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-200 outline-none focus:border-primary-500/40 transition-all" placeholder="https://cdn.empresa.com/logo.png" />
                </div>
              </div>
            </div>
      ...
      {suspendTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative glass-panel-strong rounded-2xl w-full max-w-sm p-6 animate-scale-in text-center">
            ...
            <h3 className="text-base font-semibold text-foreground-200 mb-2">Suspender tenant</h3>
            <p className="text-sm text-foreground-500 mb-6">
              Suspender <strong className="text-foreground-300">{suspendTarget.name}</strong> bloqueara el acceso para todos los usuarios que pertenecen a este tenant.
            </p>
            <div className="flex items-center justify-center gap-3">
              <button onClick={() => setSuspendTarget(null)} className="h-9 px-4 rounded-lg border border-secondary-500/20 text-sm text-foreground-400 hover:text-foreground-200 transition-all whitespace-nowrap">Cancelar</button>
              <button onClick={handleSuspend} className="h-9 px-4 rounded-lg bg-amber-600 hover:bg-amber-700 text-white transition-colors text-sm font-medium whitespace-nowrap">Suspender</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative glass-panel-strong rounded-2xl w-full max-w-sm p-6 animate-scale-in text-center">
            ...
            <h3 className="text-base font-semibold text-foreground-200 mb-2">Archivar tenant</h3>
            <p className="text-sm text-foreground-500 mb-6">
              Esta accion archivara <strong className="text-foreground-300">{deleteTarget.name}</strong>. Los datos permaneceran almacenados pero seran inaccesibles.
            </p>
            <div className="flex items-center justify-center gap-3">
              <button onClick={() => setDeleteTarget(null)} className="h-9 px-4 rounded-lg border border-secondary-500/20 text-sm text-foreground-400 hover:text-foreground-200 transition-all whitespace-nowrap">Cancelar</button>
              <button onClick={handleDelete} className="h-9 px-4 rounded-lg bg-red-600 hover:bg-red-700 text-white transition-colors text-sm font-medium whitespace-nowrap">Archivar tenant</button>
            </div>
          </div>
        </div>
      )}
