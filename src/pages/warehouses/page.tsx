...
export default function WarehousesPage() {
  ...
  const openCreate = () => {
    setFormData({ name: '', code: '', address: '', country_id: '', tenant_id: '' });
    setFormError('');
    setEditing(null);
    setShowModal(true);
  };

  const openEdit = async (w: WarehouseWithDetails) => {
    setFormData({
      name: w.name,
      code: w.code,
      address: w.address || '',
    ...
    await loadTenantsByCountry(w.country_id);
    setFormError('');
    setEditing(w);
    setShowModal(true);
  };
  ...
  const handleCountryChange = async (countryId: string) => {
    setFormData({ ...formData, country_id: countryId, tenant_id: '' });
  ...
  const handleSave = async () => {
    if (
      ...
      setFormError('Nombre, codigo, pais y tenant son requeridos');
      return;
    }
    setSaving(true);
    setFormError('');
    const result = editing
      ? await editWarehouse(editing.id, {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            },
            {
              ...
              value: new Set(warehouses.map((w) => w.country_id)).size,
              icon: 'ri-global-line',
            ...
            },
            {
              ...
              value: warehouses.reduce((s, w) => s + w.client_count, 0),
        ...
        <div className="glass-panel rounded-2xl overflow-hidden">
              ...
              <tbody>
                {filtered.map((w) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-2xs font-medium bg-primary-500/10 text-primary-400 border border-primary-500/20">
                        {w.tenant_name}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className="text-xs text-foreground-400 max-w-[200px] line-clamp-1">
                        {w.address || '—'}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
          ...
          <div className="p-4 border-t border-secondary-500/10 flex items-center justify-between">
            <span className="text-xs text-foreground-600">
              {filtered.length} de {warehouses.length} almacenes
            </span>
          </div>
        </div>
      </div>

      {/* Create/Edit Modal — Cascada País → Tenant */}
      {showModal && (
          ...
          <div className="relative glass-panel-strong rounded-2xl w-full max-w-lg p-6 animate-scale-in">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-foreground-200">
                {editing ? 'Editar almacen' : 'Nuevo almacen'}
              </h2>
              <button
                onClick={() => setShowModal(false)}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-foreground-500 hover:text-foreground-200 hover:bg-background-200/50 transition-all"
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  {formData.country_id && tenants.length === 0 && (
                    <p className="text-2xs text-amber-400 mt-1">
                      No hay tenants asociados a este pais
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej: Centro de Distribucion Central"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-foreground-400 mb-1.5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full h-10 bg-background-100 border border-secondary-500/20 rounded-lg px-3 text-sm text-foreground-200 outline-none focus:border-primary-500/40 transition-all"
                  placeholder="Ej: Autopista General Canas, Alajuela 20101"
                />
              </div>
            </div>
