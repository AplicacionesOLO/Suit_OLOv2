...
const emptyForm: FormData = {
  ...
  countryId: '', tenantId: '', warehouseId: '', clientId: '',
};
...
export default function ApplicationsPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [catResult, appResult, coRes, tRes, whRes, clRes, tcRes] = await Promise.all([
        fetchCategories(),
        fetchApplications(),
        fetchCountries(),
        fetchTenants(),
      ...
      if (appResult.data.length > 0) {
        ...
        const mapped = appResult.data.map((a: ApplicationEnriched): AppItem => {
          ...
          return {
            ...
            integrationLabel: labels[a.integration_type] || 'Interna',
            tags: a.tags || [], deletedAt: a.deleted_at,
          };
        });
        setAllApps(mapped);
      } else {
  ...
  const tenantsByCountry = useMemo(() => {
    if (!form.countryId) return tenants;
    const allowedTenantIds = new Set(tenantCountries.filter((tc) => tc.country_id === form.countryId).map((tc) => tc.tenant_id));
    return tenants.filter((t) => allowedTenantIds.has(t.id));
  }, [form.countryId, tenants, tenantCountries]);

  const warehousesByCountryTenant = useMemo(() => {
    if (!form.countryId || !form.tenantId) return [];
    return warehouses.filter((w) => w.country_id === form.countryId && w.tenant_id === form.tenantId);
  }, [form.countryId, form.tenantId, warehouses]);

  const clientsByWarehouse = useMemo(() => {
    if (!form.warehouseId) return [];
    return clients.filter((c) => c.warehouse_id === form.warehouseId);
  }, [form.warehouseId, clients]);

  // Reset downstream when upstream changes
  const handleCountryChange = (countryId: string) => {
    setForm((f) => ({ ...f, countryId, tenantId: '', warehouseId: '', clientId: '' }));
  };
  const handleTenantChange = (tenantId: string) => {
    setForm((f) => ({ ...f, tenantId, warehouseId: '', clientId: '' }));
  };
  const handleWarehouseChange = (warehouseId: string) => {
    setForm((f) => ({ ...f, warehouseId, clientId: '' }));
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (!editingId) {
      if (!form.countryId) errors.countryId = 'Selecciona un país';
      if (!form.tenantId) errors.tenantId = 'Selecciona un tenant';
      if (!form.warehouseId) errors.warehouseId = 'Selecciona un almacén';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        const { error } = await updateApplication(editingId, {
          ...basePayload,
          client_id: form.clientId || null,
        } as UpdateApplicationPayload);
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-secondary-500/10">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-medium text-foreground-500 uppercase tracking-wider">Almacén</th>
              ...
              <tbody>
                {visibleApps.map((app) => {
                  ...
                  return (
                    <tr key={app.id} className={`border-b border-secondary-500/5 hover:bg-background-100/50 transition-colors ${selectedAppId === app.id ? 'bg-primary-500/5' : ''} ${isDeleted ? 'opacity-50' : ''}`}>
                      ...
                      <td className="px-5 py-3.5"><span className="text-sm text-foreground-300">{app.clientName || '—'}</span></td>
                      <td className="px-5 py-3.5"><span className="text-sm text-foreground-400">{app.warehouseName || '—'}</span></td>
                      <td className="px-5 py-3.5"><span className="text-sm text-foreground-400">{app.tenantName || '—'}</span></td>
                      <td className="px-5 py-3.5"><span className="text-sm text-foreground-400">{app.countryName || '—'}</span></td>
                      <td className="px-5 py-3.5">
                ...
                {visibleApps.length === 0 && (
                  <tr><td colSpan={8} className="px-5 py-12 text-center text-sm text-foreground-500">{showDeleted ? 'No hay aplicaciones eliminadas.' : 'No se encontraron aplicaciones.'}</td></tr>
                )}
              </tbody>
            </table>
          </div>
            ...
            <div className="flex items-start gap-4 mb-6">
              ...
              <div className="flex-1">
                ...
                {selectedApp.clientName && (
                  <p className="text-xs text-foreground-500 mt-1">{selectedApp.clientName} · {selectedApp.tenantName}{selectedApp.warehouseName ? ` · ${selectedApp.warehouseName}` : ''} · {selectedApp.countryName}</p>
                )}
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative glass-panel-strong rounded-2xl w-full max-w-lg p-6 animate-scale-in max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                {/* Cascade: País → Tenant → Almacén → Cliente */}
                {!editingId && (
                  <>
                    <div className="p-3 rounded-xl bg-accent-500/5 border border-accent-500/10">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select value={form.countryId} onChange={(e) => handleCountryChange(e.target.value)} className={`w-full h-9 bg-background-100 border rounded-lg px-2.5 text-sm text-foreground-300 outline-none focus:border-primary-500/40 ${formErrors.countryId ? 'border-red-400' : 'border-secondary-500/20'}`}>
                        ...
                        <div>
                          ...
                          <select value={form.tenantId} onChange={(e) => handleTenantChange(e.target.value)} disabled={!form.countryId} className={`w-full h-9 bg-background-100 border rounded-lg px-2.5 text-sm text-foreground-300 outline-none focus:border-primary-500/40 disabled:opacity-40 ${formErrors.tenantId ? 'border-red-400' : 'border-secondary-500/20'}`}>
                            <option value="">Seleccionar...</option>
                        ...
                        <div>
                          ...
                          <select value={form.warehouseId} onChange={(e) => handleWarehouseChange(e.target.value)} disabled={!form.tenantId} className={`w-full h-9 bg-background-100 border rounded-lg px-2.5 text-sm text-foreground-300 outline-none focus:border-primary-500/40 disabled:opacity-40 ${formErrors.warehouseId ? 'border-red-400' : 'border-secondary-500/20'}`}>
                            <option value="">Seleccionar...</option>
                        ...
                        <div>
                          ...
                          <select value={form.clientId} onChange={(e) => setForm((f) => ({ ...f, clientId: e.target.value }))} disabled={!form.warehouseId} className={`w-full h-9 bg-background-100 border rounded-lg px-2.5 text-sm text-foreground-300 outline-none focus:border-primary-500/40 disabled:opacity-40 ${formErrors.clientId ? 'border-red-400' : 'border-secondary-500/20'}`}>
                            <option value="">Seleccionar...</option>
